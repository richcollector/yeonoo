...
<script>
		...
		xhr.onreadystatechange = function () {
			if(xhr.readyState == 4 && xhr.status == 200){				
				...
				if (jsonObj.startPage <= 1){
					...
					inA1.classList.add("page-link");
					inA1.setAttribute("aria-label","Previous");					
					inA1.setAttribute("href","javascript:vanillaStockListPagenation("+(jsonObj.startPage-1)+",'"+jsonObj.additionalParamType+"','"+jsonObj.additionalParamWord+"');");
					inLi1.appendChild(inA1);
				...
				for(var i = jsonObj.startPage ; i <= jsonObj.endPage ; i++){
					...
					if(i == jsonObj.currentPageNum){
						...
						inA2.setAttribute("href","javascript:vanillaStockListPagenation("+i+",'"+jsonObj.additionalParamType+"','"+jsonObj.additionalParamWord+"');");
						inA2.innerText = i;
				...
				if(jsonObj.endPage >= jsonObj.totalPageCount){
					...
					var inA3 = document.createElement("a");
					inA3.classList.add("page-link");
					inA3.setAttribute("href","javascript:vanillaStockListPagenation("+(jsonObj.endPage+1)+",'"+jsonObj.additionalParamType+"','"+jsonObj.additionalParamWord+"');");
					inA3.setAttribute("aria-label","Next");
					inLi3.appendChild(inA3);
	...
	function vanillaStockListPagenation(pageNumVal){
		...
		xhr.onreadystatechange = function () {
			...
			xhr.open("get" , "./vanillaStockList?pageNum=" + pageNumVal);
			//xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			xhr.send();
